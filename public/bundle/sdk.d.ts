...
export interface Link {
	 ...
	 * sdk.Link.setNavigationLinkPolicy(sdk.Link.OpenPolicy.NEW_WINDOW, 'https://example.com/${m}/show.html?');
	 *
	 ...
	 *
	 * ```typescript
	 * sdk.Link.setSameOriginLinkPolicy(sdk.Link.OpenPolicy.REPLACE);
...
export interface Pointer {
	...
	/**
	 * Register a texture image to be used with subsequent [[Pointer.editTextre]] calls.
	 * You can register multiple textures by calling this function multiple times with unique texture ids.
	 *
	 * **Note**: It is recommended to host your own images to mitigate cross origin limitations.
	 *
	 * ```typescript
	 ...
	 * ```
	 * @param textureId A user specified string to use as a lookup of this texture.
	 * @param textureSrc The src of the texture, like the src of an <img>.
	 *
	 * @embed
	 * @bundle 3.1.55.2-34-ga9934ccd93
	 */
...
export declare namespace Scene {
	 ...
	 */
	interface IComponentEventSpy<T = unknown> {
		/**
	 ...
	 */
	interface OutputPath<T = unknown> {
		get(): T;
	}
	/**
	 ...
	 */
	interface ISceneObjectSpy<T = unknown> {
		/**
		 * The path to spy on
		 */
...
export declare namespace MpSdk {
	...
	export { App, Camera, Conversion, Floor, Graph, Label, Link, Mattertag, Measurements, Mode, Model, OAuth, Pointer, Renderer, Room, Scene, Sensor, Settings, Sweep, Tour, };
}
/**
